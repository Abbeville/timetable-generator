...
//        jQuery('#modal_profile').modal('show', {backdrop: 'true'});
        $.get("http://localhost/pr-timetable/allclass/add", {headless: true}, function (resp) {
...
//        jQuery('#modal_profile').modal('show', {backdrop: 'true'});
        $.get("http://localhost/pr-timetable/allclass/edit/" + class_id, {headless: true}, function (resp) {
//            jQuery('#modal_profile .modal-body').html(resp);
            popShiiUp("Edit Class", resp);
        });
    }

    function delete_class(class_id, name) {
        class_id = $.trim(class_id);
        alertify.confirm("Delete Student", "Do You Really Want To Delete " + name + "?", function () {
        $.post("http://localhost/pr-timetable/allclass/delete/", {which: class_id}, function (resp) {

        if (resp !== "fail") {
        window.location = "http://localhost/pr-timetable/allclass/class_list/" + resp;
        }
        });
        }, function () {
